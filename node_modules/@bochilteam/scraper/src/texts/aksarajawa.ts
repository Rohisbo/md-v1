...
const isDigit = (a: string) => findstr('0123456789', a)
const isPunct = (a: string) => findstr(',.><?/+=-_}{[]*&^%$#@!~`\"\\|:;()', a)
const isVowel = (a: string) => findstr('AaĂăEeÈèÉéIiOoUuÊêĚěĔĕṚṛXxôâāīūōåɔə', a)
const isConsonant = (a: string) => findstr('BCDfGHJKLMNPQRSTVWYZbcdfghjklmnpqrstvwyzḌḍṆṇṢṣṬṭŊŋÑñɲś', a) // Xx are vowels (pepet)
const isSpecial = (a: string) => findstr('GgHhRrYyñn', a) // untuk bikonsonan th, dh, ng (nga dan cecak), ny, -r- (cakra), -y- (pengkal), jñ/jnya (ꦘ)
const isHR = (a: string) => findstr('HhRrŊŋ', a) // untuk layar dan wignyan dan cecak ([[:en:w:Engma]])
const isLW = (a: string) => findstr('LlWw', a) // untuk panjingan ("ng" dapat diikuti "g", "r"/cakra, "y"/pengkal, dan "w" atau "l"/panjingan)
const isCJ = (a: string) => findstr('CcJj', a) // untuk anuswara -nj- dan -nc-
const GetSpecialSound = (str: string) => {
  const specialsoundMap = { "f": "ꦥ꦳꧀", "v": "ꦮ꦳꧀", "z": "ꦗ꦳꧀", "ś": "ꦯ꧀", "Q": "꧀", "q": "꧀"/*pangkon*/ }
...
class Aksara {
  ...
  private vowelPrev: boolean
  private angkaFlag: boolean
  private cecakFlag: boolean
  private diftong: boolean
  ...
  toAksara(): string {
    ...
    while (i < this.str.length) {
      if (i > 0 && isVowel(this.str[i]) && isVowel(this.str[i - 1])) { //deal with words that start with multiple vocals
        if (
          (this.str[i - 1] == 'a' && this.str[i] == 'a') ||
          (this.str[i - 1] == 'i' && this.str[i] == 'i') ||
          (this.str[i - 1] == 'u' && this.str[i] == 'u') ||
          (this.str[i - 1] == 'a' && this.str[i] == 'i') ||
          (this.str[i - 1] == 'a' && this.str[i] == 'u')
        ) {//specials
          if (i == 1 || (i > 1 && !isConsonant(this.str[i - 2]))) { //for example if starts with 'ai-'
            ...
            // }
            if (this.diftong) { //do nothing, look in matramap table
            }
          ...
          (this.str[i - 1] == 'e' || this.str[i - 1] == 'è' || this.str[i - 1] == 'é') &&
          ...
          (this.str[i - 1] == 'i') &&
          (this.str[i] == 'a' || this.str[i] == 'e' || this.str[i] == 'è' || this.str[i] == 'é' || this.str[i] == 'o' || this.str[i] == 'u')
        ) {//-y-
          this.str = this.str.substring(0, i) + 'y' + this.str.substring(i, this.str.length)

        } else if (
          (this.str[i - 1] == 'o') &&
          (this.str[i] == 'a' || this.str[i] == 'e' || this.str[i] == 'è' || this.str[i] == 'é')
        ) {//-w-
          this.str = this.str.substring(0, i) + 'w' + this.str.substring(i, this.str.length)

        } else if (
          (this.str[i - 1] == 'u') &&
          (this.str[i] == 'a' || this.str[i] == 'e' || this.str[i] == 'è' || this.str[i] == 'é' || this.str[i] == 'i' || this.str[i] == 'o')
        ) {//-y-
          this.str = this.str.substring(0, i) + 'w' + this.str.substring(i, this.str.length)
      ...
      if ((isSpecial(this.str[i]) || isLW(this.str[i]) || isCJ(this.str[i])) && !this.vowelFlag) {
        //i++;
      } else if ((this.str[i] == 'h' && this.vowelFlag) || (!isVowel(this.str[i]) && i > 0) || (this.str[i] == ' ') || isPunct(this.str[i]) || isDigit(this.str[i]) || ((i - pi) > 5)) {
        if (!isDigit(this.str[i]) && this.angkaFlag) { //turn off the flag
          ret += "꧇​";// with zws
        ...
        if (isPunct(this.str[i])) {
          if (this.str[i] == '.') {
            ...
            /* comment out, not really a good way to do brackets
                            ...
                            ret += "꧍​"; pi = i + 1;// with zws
            */
          } else if (this.str[i] == '-') {//tanda hubung
            ret += "​"; pi = i + 1;
          } else if (this.str[i] == '?' || this.str[i] == '!' || this.str[i] == '"' || this.str[i] == "'") {//tanda tanya/seru/petik
            ret += "​"; //zero-width space
            pi = i + 1;
          } else {
            ret += this.str[i]; pi = i + 1;
          }
      ...
      if (pi > 0 && isVowel(this.str[pi - 1])) {//<vowel>ngg
        this.vowelPrev = true;
      }
  ...
  private GetShift(str1: string) {
    ...
    // }
    if (this.murda)
      this.str2 = str1; //case sensitive (particularly the 8 characters of ꦛ ꦜ ꦝ ꦞ ꦠ ꦡ ꦢ ꦣ),
    ...
    if (this.str2.indexOf("th") == 0) { //suku kata diawali 'th'
      if (this.str2.indexOf("thl") == 0) { //thl-
        return { "CoreSound": "ꦛ꧀ꦭ", "len": 3 };
      } else if (this.str2.indexOf("thr") == 0) { //thr-
        return { "CoreSound": "ꦛꦿ", "len": 3 };
      } else if (this.str2.indexOf("thw") == 0) { //thw-
        return { "CoreSound": "ꦛ꧀ꦮ", "len": 3 };
      } else if (this.str2.indexOf("thy") == 0) { //thy-
        return { "CoreSound": "ꦛꦾ", "len": 3 };
      } else {
        return { "CoreSound": "ꦛ", "len": 2 };//tha
      }
    } else if (this.str2.indexOf("dh") == 0) { //suku kata diawali 'dh'
      if (this.str2.indexOf("dhl") == 0) { //dhl-
        return { "CoreSound": "ꦝ꧀ꦭ", "len": 3 };
      } else if (this.str2.indexOf("dhr") == 0) { //dhr-
        return { "CoreSound": "ꦝꦿ", "len": 3 };
      } else if (this.str2.indexOf("dhw") == 0) { //dhw-: dhwani
        return { "CoreSound": "ꦝ꧀ꦮ", "len": 3 };
      } else if (this.str2.indexOf("dhy") == 0) { //dhy-: dhyaksa
        return { "CoreSound": "ꦝꦾ", "len": 3 };
      } else {
        return { "CoreSound": "ꦝ", "len": 2 };//dha
      }
    } else if (this.str2.indexOf("Th") == 0) { //suku kata diawali 'Th'
      if (this.str2.indexOf("Thl") == 0) { //Thl-
      ...
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "꧀ꦲ", "len": 2 };
    }
    ...
    if (str.indexOf("ng") == 0) { //suku kata diawali 'ng'
      ...
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "꧀ꦒ", "len": 2 };
    }
    ...
    } else if (this.str2.indexOf("ry") == 0) { //layar + ya, e.g. Suryati, Wiryadi
    ...
    } else if (this.str2.indexOf("qy") == 0) { //qy -- only pengkal
      return { "CoreSound": "ꦾ", "len": 1 };
    } else if (str.indexOf("y") == 1) { //pengkal (y di posisi karakter kedua)
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "ꦾ", "len": 2 };
    }
    ...
    } else if (this.str2.indexOf("qr") == 0) { //qr -- only pasangan ra
      return { "CoreSound": "꧀ꦫ", "len": 1 };
    } else if (this.str2.indexOf("qR") == 0) { //qR -- only cakra
      return { "CoreSound": "ꦿ", "len": 1 };
    } else if (str.indexOf("r") == 1) { //cakra (r di posisi karakter kedua)
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "ꦿ", "len": 2 };
    }
    ...
    if (str.indexOf("hl") == 0) { //wignyan + la
      ...
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "꧀ꦭ", "len": 2 };
    }
    ...
    if (str.indexOf("hw") == 0) { //wignyan + ha
      ...
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "꧀ꦮ", "len": 2 };
    }
    ...
    } else if (this.str2.indexOf("qc") == 0) { //only pasangan ca
      return { "CoreSound": "꧀ꦕ", "len": 2 };
    } else if (str.indexOf("c") == 1) { //c
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "꧀ꦕ", "len": 2 };
    }
    ...
    } else if (this.str2.indexOf("qj") == 0) { //only pasangan ja
      return { "CoreSound": "꧀ꦗ", "len": 2 };
    } else if (str.indexOf("j") == 1) { //j
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "꧀ꦗ", "len": 2 };
    }
    ...
    } else if (this.str2.indexOf("qn") == 0) { //only pasangan na
      return { "CoreSound": "꧀ꦤ", "len": 2 };
    } else if (str.indexOf("ñ") == 1) { //huruf asing sih sebenarnya, kemungkinan kecil muncul
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "꧀ꦚ", "len": 2 };
    } else if (str.indexOf("n") == 1) { //
      return { "CoreSound": "" + this.GetCoreSound(this.str2[0]).CoreSound + "꧀ꦤ", "len": 2 };
    }
  ...
  private GetCoreSound(str: string) {
  ...
  private ResolveCharacterSound(c: string) {
    var str = c.toString()
    if (isDigit(c)) {
      return "" + ('꧇' + ((parseInt(c)) - 0));
    } else if (isHR(str[0])) {
  ...
  private GetSound(str: string) {
    ...
    if (str.length == 1) return this.ResolveCharacterSound(str[0]);
      ...
      if (this.str2.indexOf("nggr") == 0) { //nggr-
        if (this.vowelPrev) konsonan = "ꦁ​ꦒꦿ";//<vowel>nggr-, e.g. panggrahita
        //else if (matra = "")
        else konsonan = "ꦔ꧀ꦒꦿ";//<nonvowel>nggr-, i.e. nggronjal
      } else if (this.str2.indexOf("nggl") == 0) { //nggl-, e.g. ngglantung
        konsonan = "ꦔ꧀ꦒ꧀ꦭ";
      } else if (this.str2.indexOf("nggw") == 0) { //nggw-, e.g. munggwing
        konsonan = "ꦔ꧀ꦒ꧀ꦮ";
      } else if (this.str2.indexOf("nggy") == 0) { //nggy-, e.g. anggyat
        konsonan = "ꦔ꧀ꦒꦾ";
      } else if (this.str2.indexOf("ngg") == 0) { //ngg-
        if (this.vowelPrev) konsonan = "ꦁ​ꦒ";//<vowel>ngg-, e.g. tunggal
      ...
      } else if (this.str2.indexOf("rlx") == 0) { //r lx, e.g. pasarlxgi
        konsonan = "ꦂꦊ"; matra = "";
      } else if (this.str2.indexOf("rrx") == 0) { //r rx
        konsonan = "ꦂꦉ"; matra = "";
      } else if (this.str2.indexOf("hlx") == 0) { //h lx
        if (this.vowelPrev) { konsonan = "ꦃꦊ"; matra = ""; }
        else konsonan = "ꦲ꧀ꦭꦼ"; matra = "";
      } else if (this.str2.indexOf("hrx") == 0) { //h rx
        if (this.vowelPrev) { konsonan = "ꦃꦉ"; matra = ""; }
        else konsonan = "ꦲꦽ"; matra = "";
      } else if (this.str2.indexOf("qlx") == 0) { //just pasangan la + pepet
        konsonan = "꧀ꦭꦼ"; matra = "";
      } else if (this.str2.indexOf("qrx") == 0) { //just cakra keret
        konsonan = "ꦽ"; matra = "";

      } else if (core_sound.CoreSound == "ꦂꦂꦮ") { // -rw-
        if (this.vowelPrev) konsonan = "ꦂꦮ";//-rw- -- arwana
        else konsonan = "ꦫ꧀ꦮ";//rw- -- rwa/rwi/rwab
      } else if (core_sound.CoreSound == "ꦃꦃꦭ") { // -hl-
        if (this.vowelPrev) konsonan = "ꦃꦭ";//-hl-
        else konsonan = "ꦲ꧀ꦭ";//hlam
      } else if (core_sound.CoreSound == "ꦃꦃꦮ") { // -hw-
        if (this.vowelPrev) konsonan = "ꦃꦮ";//-hw-
        else konsonan = "ꦲ꧀ꦮ";//hwab,hwan
      } else if (core_sound.CoreSound == "ꦃꦲꦾ") { // -hy-
        if (this.vowelPrev) konsonan = "ꦃꦪ";//sembahyang
        ...
        if ((str[0] == "n" && str[1] == "y") || ((str[0] == "t" || str[0] == "d") && str[1] == "h")) {
          konsonan = this.GetCoreSound(str[0] + str[1]).CoreSound + "ꦽ"; matra = "";//nyrê-, thrê-, dhrê-
        } else if (str[0] == "n" && str[1] == "g") {
          if (str[2] == "g") konsonan = "ꦔ꧀ꦒꦽ"; else konsonan = "ꦔꦽ"; matra = "";//nggrê-/ngrê-
        } else {
          konsonan = this.GetCoreSound(str[0]).CoreSound + "ꦽ"; matra = "";//-rê-
        }
      } else if (findstr(core_sound.CoreSound, 'ꦭ') && matra == "ꦼ") { // nga lelet
        if ((str[0] == "n" && str[1] == "y") || ((str[0] == "t" || str[0] == "d") && str[1] == "h")) {
          konsonan = this.GetCoreSound(str[0] + str[1]).CoreSound + "꧀ꦭꦼ"; matra = "";//nylê-, thlê-, dhlê-
        } else if (str[0] == "n" && str[1] == "g") {
          if (str[2] == "g") konsonan = "ꦔ꧀ꦒ꧀ꦭꦼ"; else konsonan = "ꦔ꧀ꦭꦼ"; matra = "";//ngglê-/nglê-
        } else if (str[0] == "l") {
          konsonan = "ꦊ"; matra = "";//-lê-
        } else {
          konsonan = this.GetCoreSound(str[0]).CoreSound + "꧀ꦭꦼ"; matra = "";//-lê-
        }
  ...
  toLatin() {
    ...
    // }
    const ganti = (index: number, character: string, str: string) => str.substr(0, index) + character
    ...
    for (var i = 0, j = 0; i < this.str.length; i++) {
      if (!regexp_file[this.str[i]]) { //not Aksara Jawa
        ...
        if (this.str[i] == "ꦴ" ||
          ...
          if (i > 2 && this.str[i - 1] == "ꦲ" && this.str[i - 2] == "ꦲ") { //-hah-
            ...
            (this.str[i] == "ꦶ" || this.str[i] == "ꦸ" || this.str[i] == "ꦺ" || this.str[i] == "ꦼ") &&
            (this.str[i - 1] == "ꦄ" || this.str[i - 1] == "ꦌ" || this.str[i - 1] == "ꦆ" || this.str[i - 1] == "ꦎ" || this.str[i - 1] == "ꦈ")
          ) {
          ...
          trans = ganti2(j, regexp_file[this.str[i]], trans)
          j++
        } else if (this.str[i] == "꦳") {//2 aksara -> 2 huruf
          ...
          if (i > 0 && this.str[i - 1] == "꧊") {
            ...
            (
              i == 0 ||
              [" ", "​", "꧀", "꦳", "ꦴ", "ꦶ", "ꦷ", "ꦸ", "ꦹ", "ꦺ", "ꦻ", "ꦼ", "ꦽ", "ꦾ", "ꦿ"].indexOf(this.str[i - 1]) >= 0
            )
            ...
            trans = ganti(j, regexp_file[this.str[i]], trans); j += 3;
          } else {//ba, ca, da, dll.
            trans = ganti(j, regexp_file[this.str[i]], trans); j += 2;
          }
        } else if (
          this.str[i] == "ꦔ" ||
          this.str[i] == "ꦘ" ||
        ...
        } else if (this.str[i] == "꧊") { //penanda nama diri -- made up for Latin back-compat
          trans = ganti(j, "", trans)
        } else if (this.str[i] == " ", trans) {
          if (i > 0 && this.str[i - 1] == "꧀") { //pangkon diikuti regular space = koma
            trans = ganti(j, ", ", trans)
            j += 2;
          } else {
            trans = ganti(j, " ", trans)
          ...
          if (i > 0 && this.str[i - 1] == "꧀") { //pangkon diikuti pada lingsa = titik
            trans = ganti(j, ".", trans)
            j++
