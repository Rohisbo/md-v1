...
async function wikipedia(query, lang = 'id') {
    ...
    $('#mw-content-text > div.mw-parser-output').children().map(function () {
        if (/p|h[2-4]|div/.test(this.name) && !end) {
            ...
            let el = $(this);
            if (h && /referen|Примечания|Notes_et_références/i.test(el.find('span.mw-headline').attr('id')))
                return end = true;
            let math = $(this).find('span.mwe-math-element');
            if (math.length)
                math.replaceWith($(`<span>${math.text().trim().replace(/(.*displaystyle.*|\\n)/, '')}</span>`));
            if (div && el.hasClass('thumb') && el.find('div.thumbinner > a > img[src]').length)
...
async function getHtml(lang, query) {
    ...
    let res = await (0, node_fetch_1.default)(defaultLink + '/wiki/' + query);
    if (!(res.status == 404))
    ...
    res = await (0, node_fetch_1.default)(link);
    let html = await res.text();
    const $ = cheerio_1.default.load(html);
    let results = [];
    $('ul.mw-search-results > li.mw-search-result').each(function () {
        var _a;
        let link = (_a = $(this).find('div.mw-search-result-heading > a[href]').attr('href')) === null || _a === void 0 ? void 0 : _a.trim();
        if (link)
            results.push(encodeURI(link));
    });
    if (results[0])
        return await (await ((0, node_fetch_1.default)(defaultLink + results[0]))).text();
    throw '404 Not Found!!';
}
